...
class EasyRedmineKnowledgebaseConverter extends SimpleHandler {
	...
	/**
	 * Guess if content is already in Textile format
	 *
	 * @param string $content
	 * @return bool
	 */
	public function isTextileContent( $content ) {
		// Common Textile format indicators
		$patterns = [
			// Headers (h1-h6)
			'/^h[1-6]\. .+/m',
			// Bold
		...
		// Also check for HTML tags, which would indicate non-Textile content
		preg_match_all( '/<[a-z][^>]*>/i', $content, $htmlMatches );
		$htmlCount = count( $htmlMatches[0] );
		// If we have several Textile patterns and few HTML tags, it's likely Textile
		return $matchCount > 3 && $htmlCount < $matchCount / 2;
	}
	...
	public function preprocess( $content ) {
		...
		// pre-convert anchor-wrapped img tags
		$content = preg_replace_callback(
			'/<a\s+href="([^"]+)"[^>]*>[\s\n\r]*<img([^>]*?)src="([^"]+)"([^>]*?)>[\s\n\r]*<\/a>/i',
			static function ( $matches ) {
				$href = $matches[1];
	...
	public function preHandleUrls( $content ) {
		...
		// todo: make sure that no brackets are wrapped in brackets
	...
	private function correspondUrls( $url, $domain ) {
		...
		if ( preg_match( '/' . preg_quote( $domain, '/' ) . '\/projects\/([^?\s]+)/i', $url, $matches ) ) {
			...
			if ( count( $parts ) === 1 && strpos( $parts[0], ':' ) !== false ) {
				...
				return "RMWiki-" . $parts[0] . ':' . array_pop( $parts );
			}
			return false;
		}
		if ( preg_match( '/' . preg_quote( $domain, '/' ) . '\/attachments\/([^"\'\s>\xa0]+)/i', $url, $matches ) ) {
		...
		if ( preg_match( '/' . preg_quote( $domain, '/' ) . '\/easy_knowledge_stories\/(\d+)/i', $url, $matches ) ) {
			...
			return $this->toolbox->getFormattedTitleFromId( $storyId ) ?? "EKBStory:$storyId";
	...
	public function findLinks( $content, $pageId, $version ) {
		$wikiPages = $this->dataBuckets->getBucketData( 'wiki-pages' );
		$rmwPages = $this->dataBuckets->getBucketData( 'rmw_pages' );
		$currentPage = $wikiPages[$pageId];
		$content = preg_replace_callback(
			'/\[\[([^\]|]+)(?:\|([^\]]+))?\]\]/i',
			function ( $matches ) use ( $wikiPages, $rmwPages, $pageId, $version, $currentPage ) {
				return $this->matchWikiLink(
					$matches, $wikiPages, $rmwPages, $pageId, $version, $currentPage
				);
			},
			$content
		);
	 ...
	 * @param array $rmwPages
	 * @param int $pageId
	 * @param int $version
	 * @param array $currentPage
	 * @return string
	 */
	protected function matchWikiLink( $matches, $wikiPages, $rmwPages, $pageId, $version, $currentPage ) {
		...
		if ( preg_match( '/^([^:]+):(.+)$/', $title, $nsParts ) ) {
			$namespace = $nsParts[1];
			if ( strpos( $namespace, 'RMWiki-' ) === 0 ) {
				$namespace = str_replace( 'RMWiki-', '', $namespace );
			}
			$title = trim( $nsParts[2] );
		}
		...
		if ( $namespace === 'EKBStory' && isset( $wikiPages[$title] ) ) {
			return "[[" . $wikiPages[$title]['formatted_title'] . "]]";
		}
