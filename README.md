...
3. Make sure that your MediaWiki installation supports the file extensions you are about to import: set up [$wgFileExtensions](https://www.mediawiki.org/wiki/Manual:$wgFileExtensions) properly or [configure them in the admin area](https://en.wiki.bluespice.com/wiki/Manual:Extension/BlueSpiceConfigManager#System). You can generate a list of extensions with this shell call: `find /tmp/result/images -type f -printf '%f\n' | sed 's/.*\.//' | sort | uniq`
4. To allow all existing attachments, you will need to set the following lines in your `LocalSettings.php` before the next step. ***Note that these lines disable important security features of MediaWiki, which is especially dangerous for websites on the Internet. You should remove the lines immediately after the import.***
```php
...
6. Remove the settings bypassing security features, if you have added any
7. Use `php maintenance/importDump.php /tmp/result/0-output.xml` to import the actual pages
...
## troubleshooting
1. To run this migration script, you can either use a seperate MySQL service (e.g within a docker container) that has imported a database dump of your Redmine, or  the actual database you use for your Redmine installation.
